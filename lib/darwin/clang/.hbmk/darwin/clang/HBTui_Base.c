 ...
 * Generated C source from "src/HBTui_Base.prg"
 */
...
HB_INIT_SYMBOLS_BEGIN( hb_vm_SymbolInit_HBTUI_BASE )
{ "HBTUI_BASE", {HB_FS_PUBLIC | HB_FS_LOCAL}, {HB_FUNCNAME( HBTUI_BASE )}, NULL },
...
HB_INIT_SYMBOLS_EX_END( hb_vm_SymbolInit_HBTUI_BASE, "src/HBTui_Base.prg", 0x0, 0x0003 )

#if defined( HB_PRAGMA_STARTUP )
   #pragma startup hb_vm_SymbolInit_HBTUI_BASE
#elif defined( HB_DATASEG_STARTUP )
   #define HB_DATASEG_BODY    HB_DATASEG_FUNC( hb_vm_SymbolInit_HBTUI_BASE )
   #include "hbiniseg.h"
#endif
...
HB_FUNC( HBTUI_BASE )
{
   ...
   do {
	hb_xvmVFrame( 3, 0 );
	hb_xvmSFrame( symbols + 11 );
	hb_xvmModuleName( "src/HBTui_Base.prg:HBTUI_BASE" );
	hb_xvmSetLine( 7 );
	...
	hb_xvmPushStringConst( "HBTui_Base", 10 );
...
{
   do {
	hb_xvmStatics( symbols + 11, 1 );
	hb_xvmSFrame( symbols + 11 );
	hb_xvmModuleName( "src/HBTui_Base.prg:(_INITSTATICS)" );
	/* *** END PROC *** */
   } while( 0 );
}
...
{
   do {
	hb_xvmModuleName( "src/HBTui_Base.prg:(_INITLINES)" );
	hb_xvmPushStringConst( "src/HBTui_Base.prg", 18 );
	hb_xvmPushInteger( 0 );
